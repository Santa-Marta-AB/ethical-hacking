<?php
/*---------------------------------------------------------------------------------------------------------
 * SCRIPT: Expander - Generates a word list based on variations over a single word
...
echo "Adding years from 2010 to 2021...\n";
for($i=2010;$i<=2021;$i++)
{
    foreach($newList as $item)
    {
...
$count = sizeof($list); $partial = $count;
foreach($list as $item)
{
    foreach($symbols as $sym1)
    {
        foreach($symbols as $sym2)
        {
